...
def PlotNMutDist(nmutations, plotfile, title):
    ...
    pylab.gca().set_ylim([0, max(nactual) + 2])
    pylab.gca().xaxis.set_major_locator(matplotlib.ticker.MaxNLocator(5))
    pylab.gca().yaxis.set_major_locator(matplotlib.ticker.MaxNLocator(5))
...
def PlotGeneMutDist(genelength, sub_nums, indel_nums, plotfile, cumulplotfile, title):
    ...
    yticker = matplotlib.ticker.MultipleLocator(2000)
    pylab.gca().yaxis.set_major_locator(yticker)
    ymax = max(subs + indels)
    pylab.gca().set_ylim([0, ymax])
    pylab.title(title, fontsize=12)
...
def ReadClone(line, seq):
    ...
    muts = muts.split(',')
    if len(muts) == 1 and muts[0].strip() == 'no_mutations':
        return (name, [], [])
    mutations = []
    indels = []
    for mutstring in muts:
        ...
        if m:
            ...
            assert len(wt) == len(mut) == 3
            # num is codon in 1, 2, ... numbering
            if seq[3 * num - 3 : 3 * num] != wt:
...
def main():
    ...
    for n in range(1, 4):
        print "  %d nucleotide mutations: %.8f" % (n, n_nmuts[n] / denom)
    print "\nHere are the fractions of mutation types"
    for key in ['synonymous', 'nonsynonymous', 'stop codon']:
        print "  %s: %.8f" % (key, n_muttypes[key] / denom)
    nclones = len(clone_d)
    print "\nOverall summary:\n%d clones, avg. %.3f codon substitutions, avg. %.3f indels" % (nclones, len(sub_nums) / float(nclones), len(indel_nums) / float(nclones))
